...
function ReferralReview({ referral, onClose, onTreat }: { referral: ReferralRequest; onClose: () => void; onTreat: () => void }) {
  ...
  const info = getReferralIdInfo(referral);
    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-slate-950/60 backdrop-blur-sm sm:items-center sm:p-5" onClick={onClose}>
      <section role="dialog" aria-modal="true" className="flex max-h-[92dvh] w-full max-w-xl flex-col overflow-hidden rounded-t-2xl border border-border-color bg-bg-secondary shadow-2xl sm:rounded-2xl" onClick={event => event.stopPropagation()}>
        ...
        <div className="space-y-5 overflow-y-auto p-5">
          ...
          <div className="grid grid-cols-2 overflow-hidden rounded-xl border border-border-color">
            <Detail label={info.idLabel} value={info.idValue} />
            <Detail label="Relationship" value={referral.patientRelationship || 'Self'} />
            <Detail label="Age / sex" value={referral.patientAge ? `${referral.patientAge} / ${referral.patientSex}` : 'Not provided'} />
            <Detail label={info.branchLabel} value={referral.branchCenter || 'Not provided'} />
          </div>
...
export const IncomingReferrals: React.FC = () => {
  ...
  const filtered = referrals.filter(referral => {
    ...
    return (
      !query ||
      patientName(referral).toLowerCase().includes(query) ||
      info.idValue.toLowerCase().includes(query) ||
      (referral.pensionId || '').toLowerCase().includes(query) ||
      (referral.staffIdNumber || '').toLowerCase().includes(query)
    );
  });
  const treat = (ref?: ReferralRequest) => navigate('/hospital/patient-care', { state: { referralId: ref?.id } });

  return <main className="mx-auto flex w-full max-w-[1500px] flex-col gap-4 animate-fade-in">
    <label className="flex max-w-xl items-center gap-2.5 rounded-xl border border-border-color bg-bg-secondary px-3.5 py-3 shadow-sm transition focus-within:border-primary focus-within:ring-4 focus-within:ring-primary-light">
      <Search size={17} className="text-text-muted" />
      <input
        value={search}
        onChange={event => setSearch(event.target.value)}
        placeholder="Search patient name, Staff ID, or Pension ID"
        className="min-w-0 flex-1 border-0 bg-transparent text-sm text-text-primary outline-none placeholder:text-text-muted"
      />
      {search && <button onClick={() => setSearch('')} aria-label="Clear search" className="text-text-muted hover:text-text-primary"><X size={16} /></button>}
    </label>
    {filtered.length === 0 ? <EmptyState searching={Boolean(query)} /> : <section className="overflow-hidden rounded-xl border border-border-color bg-bg-secondary shadow-sm">
      <div className="hidden overflow-x-auto lg:block">
        <table className="w-full min-w-[850px] border-collapse">
          <thead>
            <tr className="border-b border-border-color bg-bg-primary/60">
              <Heading>Patient</Heading>
              <Heading>ID No.</Heading>
          ...
          <tbody>
            {filtered.map(referral => {
              const info = getReferralIdInfo(referral);
              return (
                <tr key={referral.id} className="border-b border-border-color last:border-0 hover:bg-primary-lightest">
                  <td className="px-5 py-4"><Patient referral={referral} /></td>
                  <td className="px-5 py-4">
                    <div className="flex flex-col">
                      <span className="font-mono text-xs font-bold text-text-secondary">{info.idValue}</span>
                      <span className="text-[10px] text-text-muted uppercase font-bold">{info.idLabel}</span>
                    </div>
                  </td>
                  <td className="px-5 py-4"><Priority referral={referral} /></td>
                  <td className="px-5 py-4 text-sm font-semibold text-text-secondary">{referral.branchCenter || '—'}</td>
                  <td className="px-5 py-4 text-sm text-text-muted">{formatDate(referral.updatedAt)}</td>
                  <td className="px-5 py-4"><Actions onReview={() => setSelected(referral)} onTreat={() => treat(referral)} /></td>
                </tr>
              );
            })}
          </tbody>
      ...
      <div className="divide-y divide-border-color lg:hidden">
        {filtered.map(referral => {
          const info = getReferralIdInfo(referral);
          return (
            <article key={referral.id} className="p-4">
              <div className="flex items-start justify-between gap-3"><Patient referral={referral} /><Priority referral={referral} /></div>
              <div className="mt-4 grid grid-cols-2 gap-3 border-y border-border-color py-3">
                <Detail label={info.idLabel} value={info.idValue} compact />
                <Detail label="Received" value={formatDate(referral.updatedAt)} compact />
                <Detail label={info.branchLabel} value={referral.branchCenter || '—'} compact />
                <Detail label="Age / sex" value={referral.patientAge ? `${referral.patientAge} / ${referral.patientSex}` : '—'} compact />
              </div>
              <div className="mt-4"><Actions onReview={() => setSelected(referral)} onTreat={() => treat(referral)} /></div>
            </article>
          );
        })}
      </div>
    </section>}
    {selected && <ReferralReview referral={selected} onClose={() => setSelected(null)} onTreat={() => { const ref = selected; setSelected(null); treat(ref); }} />}
  </main>;
};
...
function EmptyState({ searching }: { searching: boolean }) { return <section className="flex min-h-64 flex-col items-center justify-center rounded-xl border border-dashed border-border-color bg-bg-secondary p-6 text-center"><div className="rounded-xl bg-primary-light p-3 text-primary"><Inbox size={22} /></div><h2 className="mt-3 text-base font-extrabold text-text-primary">{searching ? 'No matching referrals' : 'No incoming referrals'}</h2><p className="mt-1 text-sm text-text-muted">{searching ? 'Try a different patient name, Staff ID, or Pension ID.' : 'Accepted referrals will appear here.'}</p></section>; }
